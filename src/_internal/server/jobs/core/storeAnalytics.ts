/**
 * Core: aggregate seller (store) analytics for the past 6 months. Input:
 * `{ sellerId }` (the Firebase Auth UID); store slug is resolved server-side
 * via `storeRepository.findByOwnerId`.
 */
...
export async function runStoreAnalytics(
  input: StoreAnalyticsInput,
  ctx: JobContext,
