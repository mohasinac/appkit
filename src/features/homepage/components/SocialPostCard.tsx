...
const PLATFORM_META: Record<SocialPlatform, { label: string; colorClass: string; icon: React.ReactNode }> = {
  instagram: {
    label: "Instagram",
    colorClass: "bg-[image:var(--appkit-gradient-instagram)]",
    icon: (
  ...
  facebook: {
    label: "Facebook",
    colorClass: "bg-[var(--appkit-color-facebook)]",
    icon: (
      <svg className="w-3 h-3" viewBox="0 0 24 24" fill="currentColor">
  ...
  tiktok: {
    label: "TikTok",
    colorClass: "bg-[var(--appkit-color-tiktok)]",
    icon: (
      <svg className="w-3 h-3" viewBox="0 0 24 24" fill="currentColor">
  ...
  deviantart: {
    label: "DeviantArt",
    colorClass: "bg-[var(--appkit-color-deviantart)]",
    icon: (
      <svg className="w-3 h-3" viewBox="0 0 24 24" fill="currentColor">
  ...
  youtube: {
    label: "YouTube",
    colorClass: "bg-[var(--appkit-color-youtube)]",
    icon: (
      <svg className="w-3 h-3" viewBox="0 0 24 24" fill="currentColor">
...
function YouTubeCard({ post, showCaption }: { post: SocialPost; showCaption: boolean }) {
  ...
  return (
    ...
    >
      ...
      {showCaption && post.caption && (
        <Stack justify="end" surface="overlay-lg" className={`pointer-events-none absolute inset-0 ${__P.p3} opacity-0 transition-opacity duration-200 group-hover:opacity-100`}>
...
export function SocialPostCard({ post, showCaption = true, showStats = true }: SocialPostCardProps) {
  if (post.platform === "youtube") {
    return <YouTubeCard post={post} showCaption={showCaption} />;
  }
  ...
  return (
    ...
    >
      ...
      <Stack justify="end" surface="overlay-xl" className={`pointer-events-none absolute inset-0 ${__P.p3} opacity-0 transition-opacity duration-200 group-hover:opacity-100`}>
        {showCaption && post.caption && (
