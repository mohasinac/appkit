                ...
                {slide.overlay && (
                  <Div layout="flex-col" align="start" justify="center" className={`${POSITION_FILL} text-left md:px-[4rem] lg:px-[8rem]`} padding="x-lg">
                    ...
                    {slide.overlay.description && (
                      <Text color="inverse" shadow="sm" className="stagger-3 !/90 mb-4 md:mb-8 max-w-2xl" mdSize="lg" lgSize="xl" size="sm">
                        {slide.overlay.description}
                      </Text>
                    )}
        ...
        <Row gap="sm" className="absolute bottom-4 left-1/2 -translate-x-1/2 z-10">
          {slides.map((_, index) => (
            <Button
              key={index}
              variant="ghost"
              className={`relative overflow-hidden rounded-full transition-all duration-500 p-[var(--appkit-space-0)] !min-h-0 ${
 index === currentSlide
