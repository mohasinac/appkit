  ...
  return (
    ...
    >
      ...
      <Stack className="sm:items-[flex-start]" align="center" gap="xs">
        <Span color="inverse" weight="bold" className="leading-none text-[var(--appkit-color-text)] dark:" size="5xl">
          {averageRating.toFixed(1)}
        </Span>
        <StarRating value={averageRating} size="md" readOnly />
        <Text size="sm" className="text-[var(--appkit-color-text-muted)]">
        ...
        {stars.map((star) => {
          ...
          return (
            <Row key={star} align="center" gap="sm">
              <Span size="xs" weight="medium" className="w-4 text-[var(--appkit-color-text-muted)] tabular-nums" align="end">
                {star}
              </Span>
              <Span size="xs" className={CLS_STAR}>★</Span>
              <Div
                className="flex-1 h-2 bg-[var(--appkit-color-surface)] overflow-hidden" rounded="full"
                role="progressbar"
                aria-valuenow={pct}
                aria-valuemin={0}
                aria-valuemax={100}
