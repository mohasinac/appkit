...
export function ReviewCard({ review, context = "general", className = "" }: ReviewCardProps) {
  ...
  return (
    ...
    >
        ...
        <Row align="start" gap="3">
          ...
          <Div className="flex-1 min-w-0">
            <Row wrap gap="sm">
              ...
              {review.verified && (
                <Span color="success" surface="success-surface" size="xs" weight="medium" rounded="full" padding="pill-xs">
                  Verified
                </Span>
              )}
              {date && (
          ...
          <Row wrap gap="sm" className="mt-3">
            {review.images.map((img, i) => (
              <Div key={i} className="relative h-16 w-16 flex-shrink-0 border border-neutral-100" rounded="lg" overflow="hidden">
                <MediaImage src={img} alt={`Review image ${i + 1}`} size="thumbnail" />
              </Div>
            ))}
          </Row>
        )}
      ...
      {hasFooter && (
        <Stack gap="xs" className="mt-3 border-t border-neutral-100" padding="t-sm">
          {showStoreLink && (
            <Link
