...
const STORE_CLASSIFIED_TYPES: readonly ListingType[] = ["classified"];
...
export function StoreClassifiedsListing({ storeId, initialData }: StoreClassifiedsListingProps) {
  ...
  return (
    <Div className="min-h-[200px]">
      <ListingToolbar
        ...
        searchPlaceholder="Search store classifieds…"
      ...
      {/* ── Availability scope ───────────────────────────── */}
      <Div padding="y-sm">
        <AvailabilityTabs types={STORE_CLASSIFIED_TYPES} />
      </Div>
      ...
      <Div padding="y-lg">
        <Grid cols="cards" gap="md">
          {isLoading
            ...
            : products.length === 0
              ? <Div paddingY="y-4xl" className="col-span-full text-left"><Text color="muted">No classified listings in this store yet.</Text></Div>
              : products.map((p: any) => (
                  <InteractiveProductCard
                    key={p.id}
                    product={p}
                    href={String(ROUTES.PUBLIC.CLASSIFIED_DETAIL(p.slug ?? p.id))}
                  />
                ))
          }
        </Grid>
      ...
      <FilterDrawer
        ...
        hideTrigger
      >
        <ClassifiedFilters table={pendingTable as any} currencyPrefix="₹" />
      </FilterDrawer>
    </Div>
  );
}
