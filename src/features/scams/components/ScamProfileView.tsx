          ...
          <Stack gap="xs" as="ul">
            {Object.entries(CONTEST_TYPE_LABELS).map(([, label]) => (
              <Row key={label} gap="sm" align="start" as="li">
                <Span className="mt-1.5 h-1.5 w-1.5 shrink-0 bg-[color:var(--appkit-color-border,theme(colors.zinc.300))]" rounded="full" />
                    ...
                    <Stack gap="xs" as="ol">
                      {scamTypeDef.howToAvoid.map((tip, i) => (
                        <Row key={i} gap="sm" align="start" as="li">
                          <Span size="xs" weight="bold" className="flex h-5 w-5 shrink-0 items-center justify-center bg-[color:var(--appkit-color-success,theme(colors.green.600))]/10 text-[color:var(--appkit-color-success,theme(colors.green.700))]" rounded="full">
                            {i + 1}
                          </Span>
