...
const rawTesterChecklistItems: Partial<TesterChecklistItemDocument>[] = [
  ...
  ...group("buying", "Buying", [
    ...
    {
      pageKey: "order-status-lifecycle",
      pageLabel: "Order statuses & transitions",
      href: "/user/orders",
      cases: [
        { key: "all-statuses-render", label: "Every order status renders with its own readable badge, and no order shows a blank or raw status value", href: "/user/orders" },
        {
          key: "active-closed-all-tabs",
          label: "The Active / Closed / All tabs each return the right orders, and every status belongs to exactly one of the first two",
          description:
            "An order in neither tab is invisible to the buyer while still existing. Check every status you can find lands in one of them.",
          href: "/user/orders",
        },
        {
          key: "return-requested-is-active",
          label: "An order in Return Requested appears under Active, not Closed",
          description:
            "A return request is the next action item, not an ending \u2014 filing it under Closed hides the orders that most need attention.",
          href: "/user/orders",
        },
        { key: "status-change-visible-to-buyer", label: "A status change made by the seller shows on the buyer's order without the buyer having to do anything but reload", href: "/user/orders" },
        {
          key: "status-timeline-real-dates",
          label: "The order timeline shows the real recorded date for each status it reached, and a dash for steps that have no recorded date",
          description: "BEFORE: a fabricated or estimated timestamp makes the record lie. A step with no date must read as having none.",
          href: "/user/orders",
        },
        { key: "status-timeline-future-steps", label: "The timeline shows the statuses an order has not reached yet as upcoming rather than omitting them", href: "/user/orders" },
        { key: "cancel-allowed-only-before-shipping", label: "Cancelling is offered before shipping and refused afterwards, with the refusal explaining why", href: "/user/orders" },
        { key: "cancellation-reason-recorded", label: "A cancellation reason typed by the buyer is stored and visible to the seller", href: "/user/orders" },
        {
          key: "refund-appears-in-timeline",
          label: "A refund, including a partial one, appears in the order timeline rather than leaving no trace",
          description: "A partial refund changes no tracked status field, so a plain difference between two versions would record nothing at all.",
          href: "/user/orders",
        },
        { key: "tracking-number-shows-when-set", label: "Adding a tracking number and carrier makes both appear on the buyer's order with a working tracking link", href: "/user/orders" },
        {
          key: "order-scope-filter-and-status-filter",
          label: "Combining the Active/Closed tab with an explicit status filter returns a sensible result rather than nothing",
          description: "Two equality filters on one field can never both match, so the tab must stand down when an explicit status is chosen.",
          href: "/user/orders",
        },
        { key: "seller-status-controls-match-state", label: "The seller's status controls offer only the transitions valid from the order's current status", href: "/store/orders" },
        { key: "status-history-actor-recorded", label: "Each status change records who made it \u2014 buyer, seller, admin or the system \u2014 and the timeline says so", href: "/user/orders" },
        {
          key: "orders-list-shows-item-not-id",
          label: "An order row names the item it was for, with a thumbnail, rather than only an order identifier",
          description: "BEFORE: rows read \"Order {id}\" while the item title and image were already on the record, unread by the row builder.",
          href: "/user/orders",
        },
      ],
    },
    {
      pageKey: "user-uncovered-pages",
      pageLabel: "Buyer pages with no other coverage",
      href: "/user",
      cases: [
        { key: "user-preorders-renders", label: "The buyer's Pre-orders page lists their pre-orders with each one's production status", href: "/user/pre-orders" },
        { key: "user-support-new-renders", label: "Opening the new-support-ticket page directly shows the form already open, and cancelling lands back on the ticket list", href: "/user/support/new" },
        { key: "user-catalogue-new-renders", label: "Opening the new-catalogue-item page directly shows the form already open and a saved item appears in the buyer's catalogue", href: "/user/catalogue/new" },
        { key: "user-addresses-add-renders", label: "Opening the add-address page directly shows the form already open and a saved address appears in the address list", href: "/user/addresses/add" },
      ],
    },
  ]),
  ...
  ...group("selling", "Selling", [
    ...
    {
      pageKey: "listing-lifecycle",
      pageLabel: "Listing lifecycle, per type",
      href: "/store/products",
      cases: [
        { key: "standard-create-publish-sell", label: "A standard listing can be created, published, bought and marked sold, and each step shows on the public page", href: "/store/products" },
        { key: "auction-create-publish-bid-close", label: "An auction can be created with a start price and increment, published, bid on, and closes at its end time with a winner", href: "/store/products" },
        {
          key: "auction-reserve-respected-at-close",
          label: "An auction whose highest bid is below its reserve closes WITHOUT awarding a winner",
          description: "BEFORE: settlement awarded the top bid unconditionally and never read the reserve, while the reserve was displayed, editable and promised in the seller guide.",
          href: "/store/products",
        },
        { key: "preorder-create-publish-deposit", label: "A pre-order can be created with a delivery date and deposit, published, and bought at the deposit amount", href: "/store/products" },
        { key: "preorder-production-status-visible", label: "Changing a pre-order's production status shows on the public listing", href: "/store/products" },
        { key: "prizedraw-create-publish-close-reveal", label: "A prize draw can be created with entries, published, sold down and revealed with a real winner", href: "/store/products" },
        { key: "classified-create-publish-contact", label: "A classified can be created with a city and contact method, published, and offers contact rather than a cart button", href: "/store/products" },
        { key: "digitalcode-create-publish-claim", label: "A digital code listing can be created with a code pool, published, bought and fulfilled from that pool", href: "/store/products" },
        {
          key: "digitalcode-pool-depletes",
          label: "A digital-code listing whose pool empties stops being buyable, even though its stock number may not be zero",
          description: "Availability for this type is the nested codes-available count, not the stock field \u2014 the two disagree on purpose.",
          href: "/store/products",
        },
        { key: "live-create-publish-jurisdiction", label: "A live-item listing can be created with a species and jurisdiction, published, and refuses purchase from a disallowed jurisdiction", href: "/store/products" },
        { key: "art-create-publish-sell", label: "An art listing can be created with its size, material and edition fields, published, and checks out through the standard flow", href: "/store/products" },
        { key: "sticker-create-publish-sell", label: "A sticker listing can be created with its print fields, published, and checks out through the standard flow", href: "/store/products" },
        { key: "type-badge-correct-on-card", label: "Every listing type shows its own badge on a card, and no two types share one", href: "/store/products" },
        {
          key: "type-detail-route-correct",
          label: "Clicking a card of each type lands on that type's own detail page, not the standard product page",
          description: "BEFORE: three types were routed to the standard product page despite each having a working dedicated route \u2014 every card, carousel and related link sent them to the wrong page.",
          href: "/store/products",
        },
        { key: "type-survives-edit", label: "Editing a non-standard listing and saving does not silently convert it to a standard one", href: "/store/products" },
        { key: "unpublish-removes-from-public", label: "Unpublishing a listing of any type removes it from public grids while keeping it editable", href: "/store/products" },
        { key: "sold-listing-leaves-available-tab", label: "A listing that sells out moves from the Available tab to Sold & Ended without disappearing from All", href: "/store/products" },
        { key: "closed-listing-still-viewable", label: "A closed or ended listing's detail page still loads and states its outcome rather than 404ing", href: "/store/products" },
      ],
    },
    {
      pageKey: "store-uncovered-pages",
      pageLabel: "Seller pages with no other coverage",
      href: "/store",
      cases: [
        { key: "store-stickers-renders", label: "The seller's Stickers page lists their sticker listings and a new one can be created from it", href: "/store/stickers" },
        { key: "store-support-renders", label: "The seller's Support page renders and a ticket can be opened from it", href: "/store/support" },
        { key: "store-analytics-cards-renders", label: "The Analytics Cards page renders real figures rather than an empty or placeholder set", href: "/store/analytics/cards" },
        { key: "store-preorders-renders", label: "The seller's Pre-orders page lists their pre-order listings and a new one can be created from it", href: "/store/pre-orders" },
        { key: "store-guides-all-render", label: "Every seller guide page renders real content and none of them 404s", href: "/store/guide" },
        { key: "store-listing-templates-new-renders", label: "Opening the new-listing-template page directly shows the form already open and a saved template is usable when creating a listing", href: "/store/listing-templates/new" },
      ],
    },
  ]),
  ...
  ...group("content-discovery", "Content & Discovery", [
    ...
    {
      pageKey: "event-participation",
      pageLabel: "Participating in events",
      href: "/events",
      cases: [
        { key: "join-event-as-participant", label: "A buyer can join an open event and their entry is confirmed or waitlisted with the reason stated", href: "/events" },
        { key: "own-entry-visible-after-join", label: "After joining, the participant can see their OWN entry with its status, without asking anyone", href: "/events" },
        { key: "entry-survives-reload-and-relogin", label: "A participant's entry survives a reload and a sign-out and back in", href: "/events" },
        { key: "cannot-join-twice", label: "Joining the same event twice is refused rather than creating a second entry", href: "/events" },
        { key: "leave-or-cancel-entry", label: "A participant can withdraw from an event they joined, and their entry reflects it", href: "/events" },
        { key: "closed-event-cannot-be-joined", label: "An event that has ended or been cancelled cannot be joined, and says why rather than failing on submit", href: "/events" },
        { key: "leaderboard-shows-real-participants", label: "An event leaderboard lists real participants with real scores, not placeholders", href: "/events" },
        { key: "leaderboard-own-row-findable", label: "A participant can find their own row on the leaderboard without scrolling the whole list", href: "/events" },
        { key: "leaderboard-ordering-correct", label: "The leaderboard is ordered by score with ties broken consistently, and the order is the same after a reload", href: "/events" },
        { key: "leaderboard-empty-state", label: "An event with no entries shows a clear empty state on its leaderboard rather than a blank panel", href: "/events" },
        { key: "raffle-winner-announced-to-participants", label: "When a raffle is drawn, the winner is announced on the event page and the winning participant is notified", href: "/events" },
        { key: "spin-wheel-one-use-enforced", label: "A spin-wheel event enforces its per-user spin limit, and a won prize is claimable", href: "/events" },
      ],
    },
  ]),
  ...
  ...group("cta-layout", "CTA & Action Rows", [
    ...
    {
      pageKey: "navbar-ctas",
      pageLabel: "Navbar & header CTAs",
      href: "/",
      cases: [
        { key: "header-ctas-all-navigate", label: "Every control in the public header navigates somewhere real \u2014 none is inert", href: "/" },
        { key: "header-cart-badge-live", label: "The header's cart control shows a live item count that follows the cart", href: "/" },
        { key: "header-signed-out-vs-in", label: "The header offers sign-in when signed out and an account menu when signed in, and switches without a reload", href: "/" },
        { key: "account-menu-links-resolve", label: "Every item in the account menu lands on a real page for that role", href: "/" },
        { key: "role-specific-nav-entries", label: "A seller sees the store entry and a buyer does not, and an admin sees the admin entry", href: "/" },
        { key: "mobile-nav-opens-and-closes", label: "The mobile navigation opens, closes, and closes again after a link is followed", href: "/" },
        { key: "nav-active-state-correct", label: "The navigation marks the section the visitor is actually in", href: "/" },
        {
          key: "announcement-bar-message-renders",
          label: "An announcement configured by an admin appears in the announcement bar",
          description: "BEFORE: the admin editor saved the copy under a key no renderer has ever read, so announcements saved successfully and never appeared.",
          href: "/",
        },
      ],
    },
