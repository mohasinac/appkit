...
export function DigitalCodesIndexListing({ initialData }: DigitalCodesIndexListingProps) {
  ...
  return (
    <Div className="min-h-screen">
          ...
          <Div className={view === "list" ? "flex flex-col divide-y divide-zinc-100 dark:divide-zinc-800 rounded-xl border border-zinc-100 dark:border-zinc-800" : gridClass}>
            {(products as any[]).map((product) => (
              <InteractiveProductCard
                key={product.id}
                product={product}
                href={String(ROUTES.PUBLIC.DIGITAL_CODE_DETAIL(product.id))}
      ...
      <FilterDrawer
        ...
        hideTrigger
      >
        <DigitalCodeFilters table={pendingTable as any} categoryOptions={categoryOptions} />
      </FilterDrawer>
