...
const COLUMNS: AdminTableColumn<StickersRow>[] = [
...
export function SellerStickersView({
  onCreateClick,
  onEditClick,
  onDelete,
  onBulkDelete,
}: SellerStickersViewProps) {
  ...
  const handleEdit = useCallback(
    (id: string) => {
      ...
      else window.location.href = String(ROUTES.STORE.STICKERS_EDIT(id));
    },
    [onEditClick],
  );
  ...
  const config: ListingViewConfig<ProductsResponse, StickersRow> = {
    ...
    buildBulkActions: onBulkDelete
      ? (selection): BulkActionItem[] =>
          SELLER_BULK_ACTIONS.stickers.map((id) => ({
            id,
  ...
  return (
    <>
      ...
      {deleteTargetId && (
        <ConfirmDeleteModal
          isOpen
          title="Delete Sticker Listing"
          message="Are you sure you want to delete this sticker listing? This cannot be undone."
