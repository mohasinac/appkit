...
const { mockOrderFindById, mockOrderUpdate } = vi.hoisted(() => ({
...
// only orderRepository matters for markEmiInstallmentPaid, so the rest resolve
// to their real implementations (the file must still import cleanly).
...
describe("markEmiInstallmentPaid", () => {
  it("order not found → throws", async () => {
    ...
    await expect(
      markEmiInstallmentPaid("store-A", "seller", "order-1", { installmentIndex: 1 }),
  ...
  it("installment index not found → throws", async () => {
    ...
    await expect(
      markEmiInstallmentPaid("store-A", "seller", "order-1", { installmentIndex: 99 }),
    ).rejects.toThrow(/not found/i);
  });
  ...
  it("marks the target installment paid, recomputes remaining balance, emiComplete stays false when others pending", async () => {
    mockOrderFindById.mockResolvedValue(makeOrder());
    await markEmiInstallmentPaid("store-A", "seller", "order-1", {
      installmentIndex: 1,
  ...
  it("marking the last pending installment paid → emiComplete becomes true, remaining balance 0", async () => {
    ...
    await markEmiInstallmentPaid("store-A", "seller", "order-1", { installmentIndex: 3 });
