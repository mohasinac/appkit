...
import { BulkActionBar, Div, FilterDrawer, ListingToolbar, LoginRequiredModal, Pagination, Row, Text } from "../../../ui";
...
export function ClassifiedIndexListing({ initialData }: ClassifiedIndexListingProps) {
  ...
  return (
    <Div className="min-h-screen">
          ...
          <Div className={view === "list" ? "flex flex-col divide-y divide-zinc-100 dark:divide-zinc-800 rounded-xl border border-zinc-100 dark:border-zinc-800" : gridClass}>
            {(products as any[]).map((product) => (
              <InteractiveProductCard
                key={product.id}
                product={product}
                href={String(ROUTES.PUBLIC.CLASSIFIED_DETAIL(product.id))}
      ...
      <FilterDrawer
        ...
        hideTrigger
      >
        <ClassifiedFilters table={pendingTable as any} categoryOptions={categoryOptions} />
      </FilterDrawer>
