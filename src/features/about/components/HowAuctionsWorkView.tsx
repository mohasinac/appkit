...
export async function HowAuctionsWorkView({
  ...
  return (
    <div className="-mx-4 md:-mx-6 lg:-mx-8 -mt-6 sm:-mt-8 lg:-mt-10" data-section="howauctionsworkview-div-115">
      <Section className={`${heroBannerClass} text-white py-14 md:py-16 lg:py-20`}>
        <div className={`${page.container.md} text-center`} data-section="howauctionsworkview-div-116">
      ...
      <div className={`${page.container.md} py-10 md:py-12 lg:py-16 space-y-14`} data-section="howauctionsworkview-div-117">
    ...
    <Section>
      ...
      <Stack gap="md" className="gap-5">
        {steps.map(({ number, icon, title, text }) => (
          <div key={number} className={`flex items-start gap-4 p-5 rounded-xl border ${themed.border} ${themed.bgPrimary}`} data-section="howauctionsworkview-div-118">
            <div className={`flex-shrink-0 w-10 h-10 rounded-full bg-primary/10 dark:bg-primary/15 ${flex.center} text-xl`} data-section="howauctionsworkview-div-119">{icon}</div>
            <div data-section="howauctionsworkview-div-120">
...
function renderAuctionInfoCardsSection(flex: (typeof THEME_CONSTANTS)["flex"], cards: AuctionInfoCard[]) {
  return (
    <Section>
      <div className="grid gap-5 md:grid-cols-2" data-section="howauctionsworkview-div-121">
        {cards.map(({ icon: Icon, title, text, color, iconColor }) => (
          <div key={title} className={`rounded-xl border p-5 ${color}`} data-section="howauctionsworkview-div-122">
            <div className={`w-10 h-10 rounded-lg bg-white/60 dark:bg-white/10 ${flex.center} mb-3`} data-section="howauctionsworkview-div-123">
...
function renderAuctionCtaSection(t: AuctionT, themed: (typeof THEME_CONSTANTS)["themed"], flex: (typeof THEME_CONSTANTS)["flex"]) {
  return (
    <Section className={`rounded-2xl p-8 text-center ${themed.bgSecondary} border ${themed.border}`}>
      ...
      <div className={`${flex.center} gap-4 flex-wrap`} data-section="howauctionsworkview-div-124">
