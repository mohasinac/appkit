/**
 * Generates a single index.ts with all exports named explicitly.
 * Deduplicates: first file in the priority list wins for each symbol.
 */
...
function resolveRel(fromFile, sub) {
  if (!sub.startsWith(".")) return null; // external dep, skip
...
function getDirectExports(relFile) {
  ...
  // export declare / export function / export class / export const etc.
  ...
  // export * from './x' — follow ONE level
  for (const m of text.matchAll(
...
// Priority-ordered list of files to export from.
// First file wins for any given symbol name.
const FILES = [
  // ─── tier 0 ───────────────────────────────────────────────────────────────
  "tokens/index.ts",
  // ─── tier 1 UI ────────────────────────────────────────────────────────────
...
// Single seen set — a symbol name can only appear once regardless of type vs value
const seen = new Set();
const sections = [];
...
const output = [
  "// AUTO-GENERATED — single unified barrel, do not edit manually",
  "// Run: node scripts/generate-index.mjs > src/index.ts",
  "",
  sections.join("\n"),
].join("\n");

writeFileSync(join(SRC, "index.ts"), output);
console.log("Written src/index.ts");
console.log("Total exports:", seen.size);
