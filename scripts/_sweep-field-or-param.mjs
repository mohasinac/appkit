#!/usr/bin/env node
// W16 codemod: swap `: unknown` field/param annotations to `: JsonValue`
// and `unknown[]` to `JsonArray`. Skips:
// - catch (e: unknown) clauses (audit allows these)
// - <T = unknown> generic defaults (audit allows)
// - Type guards `(x: unknown): x is Foo` — preserves (handled separately)
// - Allowlisted files
// - per-line `// audit-unknown-ok` markers
...
const ALLOWLISTED_FILES = new Set([
  ...
  "src/cli/index.ts",
  // Additional files that genuinely cannot use JsonValue (logger callers etc.)
...
function isTypeGuard(line) {
  // `: x is Foo` somewhere on the line is a type predicate
  return /:\s*\w+\s+is\s+/.test(line);
}
...
for (const file of walk(ROOT)) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (isTypeGuard(line)) continue;

    let changed = false;
    // Swap `: unknown[]` to `: JsonArray` (do this first; more specific)
    const newLine1 = line.replace(/(:\s*)unknown\[\]/g, (_, prefix) => {
      changed = true;
      needsJsonArray = true;
      sitesInFile++;
    ...
    // Swap `: unknown` field/param annotations to `: JsonValue`
    // Avoid `Promise<unknown>`, `<T = unknown>`, etc. — only standalone `: unknown\b`
    const newLine2 = line.replace(/(:\s*)unknown\b(?!\[)/g, (match, prefix, offset, full) => {
      // Skip if inside generic brackets like Promise<unknown>
      const before = full.slice(0, offset);
      const openCount = (before.match(/</g) || []).length;
      const closeCount = (before.match(/>/g) || []).length;
      if (openCount > closeCount) return match; // inside generic
      changed = true;
      needsJsonValue = true;
      sitesInFile++;
