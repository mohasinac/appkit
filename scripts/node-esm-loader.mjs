/**
 * ESM resolver hook — makes `import("@mohasinac/appkit")` work from ANY
 * standalone Node script.
 *
 * appkit is `"type": "module"` built by plain `tsc`, and tsc does not rewrite
 * import specifiers — so all ~2,190 emitted `.js` files carry bare relative
 * imports with no `.js` extension. Bundlers (Next/Turbopack, tsup, vite)
 * resolve those happily; Node's ESM resolver refuses them outright. This hook
 * retries with `.js` and then `/index.js` on ERR_MODULE_NOT_FOUND, and stubs
 * the two other things that make a raw Node import of appkit explode.
 *
 * Registered by:
 *   - `appkit/scripts/seed-cli.mjs`      (the appkit-seed CLI)
 *   - `scripts/qa/roundtrip-diff.mjs`    (the schema field-loss gate)
 *
 * **Renamed from `seed-cli-loader.mjs`** — it was never seed-specific, and
 * that name was why a later session diagnosed the same ESM failure from
 * scratch and concluded the fix was to rewrite every specifier at build time.
 * It is not: it is this file, registered in three lines. `seed-cli-loader.mjs`
 * remains as a re-export so any pinned path keeps working.
 */
