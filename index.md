...
| `isAdminUser`, `isSellerUser`, `isModeratorUser`, `isEmployeeUser`, `isBuyerUser`, `isEffectiveAdminUser` *(new, 2026-08-19)* | `features/auth/role-predicates.ts` | Canonical `role`-string predicates. `isEffectiveAdminUser(input)` = `isAdminUser(input) \|\| (input.isTester && input.canTestAdmin)` — the bypass used by `createRouteHandler`'s role/permission gates and `makeAdminSectionLayout` so a `canTestAdmin` tester gets real `/admin/**` access without a `role` change. Both gates resolve `isTester`/`canTestAdmin` via one live `userRepository.findById` (never JWT claims — only `role` is baked into the session cookie), paid only on the path that would otherwise reject. | `"@mohasinac/appkit"` + `"@mohasinac/appkit/client"` |
